...
extern int main(int argc, char **argv)
{
    ...
    if (gQueues._seperate_present_queue) {
        vkGetDeviceQueue(gDevice, present_queue_family_idx, 0, &gQueues._present);
    } else {
        vkGetDeviceQueue(gDevice, graphics_queue_family_idx, 0, &gQueues._graphics);
        gQueues._present = gQueues._graphics;
    }
    vkGetDeviceQueue(gDevice, compute_queue_family_idx, 0, &gQueues._compute);
    ...
    size_t triangle_data_size = sizeof(triangle_vertices) + sizeof(triangle_indices);
    ...
    buffer_ci.size  = triangle_data_size;
    ...
    memcpy(mapped_vertex_buffer_ptr, (void *)&triangle_vertices, sizeof(triangle_vertices));

    size_t indices_offset        = sizeof(triangle_vertices);
    void  *offset_ptr_to_indices = (uint8_t *)mapped_vertex_buffer_ptr + indices_offset;

    memcpy(offset_ptr_to_indices, (void *)&triangle_indices, sizeof(triangle_indices));
    ...
    while (!bQuit) {
        ...
        while (SDL_PollEvent(&e) != 0) {
            ...
            switch (e.type) {
                ...
                case SDL_KEYDOWN:
                    if (key == SDLK_f) SDL_SetWindowFullscreen(gWindow, (Uint32)SDL_WINDOW_FULLSCREEN_DESKTOP);
                    break;
                case SDL_QUIT:
                    bQuit = true;
        ...
        clear_value.color        = { 0.f, 1.f, 0.f, 1.f };
        ...
        rendering_info.pDepthAttachment; // todo
...

        {
            VkDeviceSize offsets[] = {
                /*POSITIONs*/ 0,
                /*COLORs   */ sizeof(float) * 9 // offset to the start of the attribute within buffer
            };
            VkBuffer buffers[] = {
                vertex_buffer,
                vertex_buffer,
            ...
            vkCmdBindVertexBuffers(graphics_cmd_buffer, 0, ARR_COUNT(buffers), buffers, offsets);
            vkCmdBindIndexBuffer(graphics_cmd_buffer, vertex_buffer, indices_offset, VK_INDEX_TYPE_UINT16);

            vkCmdDrawIndexedIndirect(graphics_cmd_buffer, indirect_commands_buffer, 0, 1, 0);
        }
